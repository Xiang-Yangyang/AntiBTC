...
describe("AntiBTC", function () {
  ...
  describe("交易功能", function () {
    ...
    it("应该允许用户卖出 AntiBTC 换取 USDT", async function () {
      ...
      const usdtBeforeSell = await mockUSDT.balanceOf(user1.address);
      
      // 用户卖出代币
      await antiBTC.connect(user1).sellTokens(tokenBalance);
      
      // 检查用户余额
      expect(await antiBTC.balanceOf(user1.address)).to.equal(0);
      expect(await mockUSDT.balanceOf(user1.address)).to.be.gt(usdtBeforeSell);
    });
  });
  ...
  describe("流动性功能", function () {
    it("应该允许用户添加流动性", async function () {
      ...
      // 记录初始状态
      const initialTokenBalance = await antiBTC.balanceOf(user2.address);
      ...
      // 检查用户余额
      expect(await antiBTC.balanceOf(user2.address)).to.be.gt(initialTokenBalance);
      
      // 检查池子状态
      expect(await antiBTC.poolTokens()).to.be.gt(initialPoolTokens);
      expect(await antiBTC.poolUSDT()).to.be.gt(initialPoolUSDT);
    });
    ...
    it("应该允许用户移除流动性", async function () {
      ...
      // 用户移除流动性
      await antiBTC.connect(user2).removeLiquidity(tokenBalance);
      
      // 检查用户余额
      expect(await antiBTC.balanceOf(user2.address)).to.equal(0);
      expect(await mockUSDT.balanceOf(user2.address)).to.be.gt(initialUSDTBalance);
    });
  });
